  ...
  function __construct() {
    ...
    */
    function process_button() {
      ...
      {
        $sql = sprintf("SELECT customers_info_date_account_created, customers_dob
                        FROM %s
      ...
      while ($check = tep_db_fetch_array($check_query)) {
         ...
         $sql = sprintf("select orders_total.value from %s join %s on orders_status.orders_status_id = orders.orders_status
         ...
         TABLE_ORDERS_STATUS, TABLE_ORDERS, TABLE_ORDERS_TOTAL, $_SESSION['customer_id']);
         $check_query = tep_db_query($sql);
