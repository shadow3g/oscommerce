...
class pagamastarde_eom {
  var $code, $title, $description, $enabled;

  /*
  * constructor
  */
  function pagamastarde_eom() {
    global $order;
    $this->signature = 'pagamastarde_eom|pagamastarde_eom|1.2|1.0';
    $this->code = 'pagamastarde_eom';
    $this->title = MODULE_PAYMENT_PAGAMASTARDE_EOM_TEXT_TITLE;
    $this->description = MODULE_PAYMENT_PAGAMASTARDE_EOM_TEXT_DESCRIPTION;
    $this->sort_order = MODULE_PAYMENT_PAGAMASTARDE_EOM_SORT_ORDER;
    $this->enabled = ((MODULE_PAYMENT_PAGAMASTARDE_EOM_STATUS == 'True') ? true : false);
    $this->mode = ((MODULE_PAYMENT_PAGAMASTARDE_EOM_MODE == 'Test') ? 'test' : 'real');

    if ((int)MODULE_PAYMENT_PAGAMASTARDE_EOM_PREPARE_ORDER_STATUS_ID > 0) {
      $this->order_status = MODULE_PAYMENT_PAGAMASTARDE_EOM_PREPARE_ORDER_STATUS_ID;
    }
    ...
    */
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PAGAMASTARDE_EOM_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAGAMASTARDE_EOM_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    */
    function selection() {
      ...
      'module' => MODULE_PAYMENT_PAGAMASTARDE_EOM_SHOP_TEXT_TITLE);
    }
    ...
    */
    function process_button() {
      ...
      if ( $this->mode == 'real'){
        $pagamastarde_account_id = trim( MODULE_PAYMENT_PAGAMASTARDE_EOM_ACCOUNT_ID );
        $pagamastarde_secret = trim( MODULE_PAYMENT_PAGAMASTARDE_EOM_SECRET );
      }else{
        $pagamastarde_account_id = trim( MODULE_PAYMENT_PAGAMASTARDE_EOM_TEST_ACCOUNT_ID );
      ...
      if (MODULE_PAYMENT_PAGAMASTARDE_EOM_DISCOUNT == 'False'){
        $dicount="false";
      }else{
        $dicount="true";
      }

      if (MODULE_PAYMENT_PAGAMASTARDE_EOM_IFRAME == 'False'){
        $iframe="false";
      }else{
        $iframe="true";
      }
      ...
      $_SERVER['SERVER_NAME'],
      $_SERVER['REQUEST_URI'])).'/ext/modules/payment/pagamastarde/callback_eom.php';
      ...
      'amount' => $amount,
      'currency' => $currency,
      //'description' => MODULE_PAYMENT_PAGAMASTARDE_EOM_SUBJECT.' '.$current_order_id,
  ...
  */
  function before_process() {
    ...
    if (tep_db_num_rows($check_query)) {
      ...
      if ($check['orders_status'] == MODULE_PAYMENT_PAGAMASTARDE_EOM_PREPARE_ORDER_STATUS_ID) {
    ...
    tep_db_query("update " . TABLE_ORDERS . " set orders_status = '" . (MODULE_PAYMENT_PAGAMASTARDE_EOM_ORDER_STATUS_ID > 0 ? (int)MODULE_PAYMENT_PAGAMASTARDE_EOM_ORDER_STATUS_ID : (int)DEFAULT_ORDERS_STATUS_ID) . "', last_modified = now() where orders_id = '" . (int)$order_id . "'");
  ...
  function check() {
    if (!isset($this->_check)) {
      $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PAGAMASTARDE_EOM_STATUS'");
  ...
  function install() {
    ...
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Paga Más Tarde', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_STATUS', 'False', 'Do you want to accept Paga Mas Tarde payments?', '6', '3', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('TEST Public Key', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_TEST_ACCOUNT_ID', '', 'Codigo de tu cuenta de TEST de Paga Mas Tarde', '6', '4', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('TEST Secret Key', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_TEST_SECRET', '', 'Clave de firma de tu cuenta de TEST de Paga Mas Tarde.', '6', '4', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('REAL Public Key', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_ACCOUNT_ID', '', 'Codigo de tu cuenta REAL de Paga Mas Tarde', '6', '4', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('REAL Secret Key', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_SECRET', '', 'Clave de firma de tu cuenta REAL de Paga Mas Tarde.', '6', '4', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Modo de pago', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_MODE', 'Test', 'Change payment mode?', '6', '3', 'tep_cfg_select_option(array(\'Test\', \'Real\'), ', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Iframe - abrir en pop-up', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_IFRAME', 'False', 'Do you want to open form in a pop-up?', '6', '3', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Zona en la que está operativo', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_ZONE', '0', 'Si selecciona una zona, esta forma de pago solo estara disponible para dicha zona.', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Estado del pedido antes de confirmar', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_PREPARE_ORDER_STATUS_ID', '" . $status_id . "', 'El pedido se guarda inicialmente en este estado antes de que PagaMasTarde confirme el pago', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Estado del pedido confirmado', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_ORDER_STATUS_ID', '0', 'Cuando PagaMasTarde confirma el pago, el pedido pasara a este estado', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
  ...
  function keys() {
    return array('MODULE_PAYMENT_PAGAMASTARDE_EOM_STATUS','MODULE_PAYMENT_PAGAMASTARDE_EOM_MODE', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_TEST_ACCOUNT_ID', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_TEST_SECRET', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_ACCOUNT_ID', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_SECRET', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_IFRAME', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_ZONE', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_PREPARE_ORDER_STATUS_ID', 'MODULE_PAYMENT_PAGAMASTARDE_EOM_ORDER_STATUS_ID','MODULE_PAYMENT_PAGAMASTARDE_EOM_SORT_ORDER');
  }
